...
#=========================================================
#	   Replace the "game.gd" in the godot folder          
#		with this one when compiling for Switch.          
#	  It is recommended to only use this file for         
...
func do_physics():
	if get_colider(map_object_layer)>1:
		if get_colider(map_object_layer)<=4:
			...
			temp_tm=SwitchTimer.get_estimate_time_secs+4
			...
			player.speed=2
			temp_tm2=SwitchTimer.get_estimate_time_secs+5
...
func _process(_delta):
	...
	if game_over_!=true:
		...
		if spawn_hole_==false and hole_spawn_delay<=SwitchTimer.get_estimate_time_secs:
			spawn_hole_=true
		
		if hole_update_==false and hole_update_delay<=SwitchTimer.get_estimate_time_msecs:
			hole_update_=true

		
		if item_place_==false and item_delay<=SwitchTimer.get_estimate_time_secs:
			item_place_=true

		if spawn_hole_==true:
			spawn_hole()
			hole_spawn_delay=SwitchTimer.get_estimate_time_secs+4-difficulty
			spawn_hole_=false
		
		if hole_update_==true:
			update_holes(map_object_layer)
			hole_update_delay=SwitchTimer.get_estimate_time_msecs+450
			hole_update_=false

		if item_place_==true:
			add_object(map_object_layer,"item")
			item_delay=SwitchTimer.get_estimate_time_secs+7+difficulty
			item_place_=false

		if player.player_state==1:
			game_over()
		elif player.player_state==2:
			if temp_tm<=SwitchTimer.get_estimate_time_secs:
				player.player_state=0
				player.speed=1
		elif player.player_state==3:
			if temp_tm2<=SwitchTimer.get_estimate_time_secs:
				player.player_state=0
				player.speed=1 
...
func _input(event):
	if game_over_==true:
		...
		if (Input.is_action_just_pressed("in_accept") or (event is InputEventMouseButton)) and click_delay<=SwitchTimer.get_estimate_time_msecs:
